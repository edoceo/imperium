...
echo "<tr><td class='l'>Kind:</td><td colspan='2'>" . \radix_html_form::select('kind',$this->Account['kind'], $this->AccountKindList) . "</td></tr>";
echo "<tr><td class='l'>Code &amp; Name:</td><td>" . \radix_html_form::text('code', $this->Account['code'],array('size'=>6)) . '</td>';
echo '<td>' . \radix_html_form::text('name', $this->Account['name']) . '</td>';
echo '</tr>';
...
echo "<tr><td class='b r'>Tax Line:</td><td colspan='3'>" . \radix_html_form::select('account_tax_line_id', $this->Account->account_tax_line_id, $list) . "</td>";
...
echo "<tr><td class='b r'>Transit:</td><td colspan='2'>" . \radix_html_form::text('bank_routing', $this->Account->bank_routing) . '</td></tr>';
echo "<tr><td class='b r'>Account:</td><td colspan='2'>" . \radix_html_form::text('bank_account', $this->Account->bank_account) . '</td></tr>';
...
echo '<div class="cmd">';
echo \radix_html_form::hidden('id', $this->Account['id']);
...
$res = Radix\DB\SQL::fetch_one('SELECT count(id) FROM account_ledger WHERE account_id = ?', array($this->Account['id']));
