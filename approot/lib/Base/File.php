...
class Base_File extends ImperiumBase
{
    ...
    */
    function import($f)
    {
        ...
        move_uploaded_file($f,$this->path);
    }
    ...
    */
    private function _savePath()
    {
        ...
        $name = sprintf('%s/%s/%s/%s',substr($this->hash,0,2),substr($this->hash,2,2),substr($this->hash,4,2),$this->hash);
    ...
    */
    static function mimeInfo($mime)
    {
        ...
        static $tab;
        if (empty($tab)) $tab = parse_ini_file(APP_ROOT . '/approot/etc/mime.ini',true);
...
//     }
    ...
    */
    static function copyPost($f)
    {
        $bf = new self();
        $bf->name = $f['name'];
        $bf->kind = $f['type'];
        $bf->size = $f['size'];
        $bf->import($f['tmp_name']);
        return $bf;
    }
}
